...
3. Отправка личных конфигураций на сервер при необходимости

# 2.1 Типы пользователей
В системе имеется два типа пользователей: администратор и управляемое устройство
Администратор имеет доступ к web-сервису и права на запрос выполнения основных задач серверу.
Управляемое устройство имеет доступ к связи с сервером для выполнения сообщённых задач, а также имеет возможность сообщать серверу свои конфигурации
...
После регистрации у пользователя появляется возможность создавать группы и подключаться к ним
При успешном входе, в идеале, хранить куки или другую информацию для последующего автоматического входа на сервис

# 2.2 Создание групп и подключение к ним
...
2. Выполнение запросов на выключение, перезагрузку, сон устройства
3. Просмотр местоположения устройств на карте
4. Просмотр состояний (сон/подключен/отключен) устройств на момент последнего обновления данных
...
# 2.10 Интерфейс подключение устройства к группе*
